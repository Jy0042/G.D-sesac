...
// 2. 컴포넌트 내에세 props 매개변수를 구조 분해

/*
export default function FuncProps(props) {
  // props를 구조 분해 할당해서 weather와 feeling으로 바로 값 사용할 수 있도록 하기
  ...
  return (
    <>
      ...
      <div>weather props 로 전달받은 데이터 : {weather}</div>
      <div>weather props 로 전달받은 데이터 : {feeling}</div>
    </>
  );
}
*/
...
export default function FuncProps({ weather, feeling, date}) {
  ...
  return (
    <>
      <h1>함수형 컴포넌트에서 props 사용하기</h1>
      <div>default props 를 사용해 weather props만 보여주기</div>
      <div>매개변수로 props 를 받아올 때부터 구조 분해하여 사용</div>
...
// 만약 weather props 가 전달되지 않는다면 defaultProps에서 해당 값을 찾아 사용
