...
// 구조 분해하는 배열보다 변수가 더 많은 경우 배열의 길이를 넘어가는 변수는 자동으로 undefined
const [cc1, cc2, cc3, cc4, cc5] = colors;
console.log(cc5); // undefined

// default 값 미리 선언
// default 값의 경우 해당 변수에 저장할 값이 없을 때만 사용
const [
  ccc1,
  ccc2,
  ccc3 = 'ccc3 default', // default 값이 사용 되지 않고, colors 배열의 2번 인덱스인 'yellow'가 저장
  ccc4,
...
// 구조 분해 할당
// 순서에 상관 없이 구조 분해 할당하는 객체의 key와 동일한 변수에 값이 저장
const {color, name, version} = myPhone;
console.log(name, color, version);

// 다른 이름으로 변수를 할당할 수 없음
const { c, n ,v } = myPhone;
console.log(c, n, v); // key 와 변수가 동일해야 해당하는 변수에 할당

// 다른 이름으로 변수를 할당하려는 경우
const { name : nn, color : cc, version : vv } = myPhone;
console.log(nn, cc, vv);

// key가 존재하지 않는 경우를 대비해 default 값 지정
const { name : nnn, color : ccc, storage : sss = 256, version : vvv } = myPhone;
console.log(nnn, ccc, sss, vvv);
...
console.log(key5); // 5
console.log(obj); // key1, key5 를 제외한 key-value 쌍이 모두 obj 변수에 저장 

// -----------------------
// 함수에서 전개연산자와 구조 분해 할당, default 값 지정
function test(...val) {
  const [v1, v2, v3, ...rest] = val;
  console.log(v1);
  console.log(v2);
  console.log(v3);
  console.log(rest);
