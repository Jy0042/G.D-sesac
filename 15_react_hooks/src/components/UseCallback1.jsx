...
export default function UseCallback1() {
  ...
  // 그렇기 때문에 isTrue state 에 변경사항이 있어도 func1 이라는 함수는 다시 선언됨
  // useEffect에서 의존성 배열로 func1에 변경사항이 생기면 내부 로직을 실행하도록 작성 시 매번 useEffect의 콜백함수가 실행됨
  const func1 = () => {
  ...
  // number의 값이 바뀌면 useCallback hook의 첫 번째 인자로 전달한 콜백 함수를 다시 메모이제이션
...
// useEffect의 의존성 배열에 함수가 들어가는 경우 useCallback hook을 사용하면 성능에 도움이 됨
...
// 다시 선언되지 않음 - > 계속 사용하지 않는 함수나 연산이 너무 복잡해서
