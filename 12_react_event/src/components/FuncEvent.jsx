...
export default function FuncEvent() {
  ...
  return (
    <>
      ...
      {/* 함수에 매개변수를 전달하는 경우 */}
      {/* 방법 1. */}
      {/* onClick에서 익명함수는 선언하고, 그 내부에 함수를 실행시킴 */}
      ...
      {/* 방법 2. */}
      {/* bind 메서드 사용 */}
      <button onClick={alertMsg.bind(null, "끝까지 불 태워 봅시다!!👊")}>
        메세지 alert 창에 띄우기2
      </button>
      ...
      <input
        type="text"
        value={menu}
        placeholder="오늘 점심 메뉴는?"
        onChange={(e) => {
          // 여기서 사용되는 e는 브라우저의 native 이벤트 객체가 아닌 react synthetic 이벤트 객체
          // console.log(e);
